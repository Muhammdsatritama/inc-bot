...
let [query,aha]= text.split(/[&|.]/i)
 ...
 if (aha < 1 || aha > 5) {
      throw `*[permintaan error]*\n untuk menggunakan fitur ini minimal request 1 dan maksimal request 5`
    }
let res = await Pinterest(query)
for (let i = 0; i < `${aha}`; i++) {
conn.sendFile(m.chat, res[Math.floor(Math.random() * res.length)],`${query}`+'.jpg', 'ini kak, jangan lupa follow\ninstagram.com/mstm_27', m, false, { thumbnail: Buffer.alloc(0) })
await delay(2500)
}
}
 
handler.help = ['pinterest'].map(v => v + ' [gambar]|[jumlah]')
